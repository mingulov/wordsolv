...
packages/semantic-core/
  ...
  bin/
    ...
    build-probes.ts   farthest-point probe ladder generation
...
### 6.3 Cold-start probe ladder

The weakest area, and where the first attempt failed: k-means over a frequency-sorted
pool selected function words and junk. The spec therefore requires:

1. restrict probe candidates to **concrete, common, provider-valid** nouns
2. select by **farthest-point sampling** in embedding space, not cluster frequency
3. **validate against the gold fixture** — the acceptance metric is the distribution of
   best-rank-achieved after *k* probes, and the ladder must beat random common-noun
   selection by a stated margin

Ordered by expected information gain so the user can stop early. Generated by
...
- one-tap marking of a rejected word
...
3. **Cold start is unsolved.** With no guess inside the game's top-300 the solver is
   near-useless (answer at ~#20,000). The probe ladder is a mitigation whose value is
   asserted, not yet measured — §6.3 makes measuring it an acceptance criterion.
...
1. Is there a Russian embedding closer to the game's than araneum's ρ=0.65? A bounded
   search is the single highest-leverage possible improvement.
